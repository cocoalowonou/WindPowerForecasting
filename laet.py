...
class LTAE(nn.Module):
    def __init__(self, in_channels=128, n_head=16, d_k=8, n_neurons=[256,128], dropout=0.2, d_model=256,
                 T=1000, len_max_seq=134, positions=None, return_att=True):
        """
        ...
        if d_model is not None:
            ...
            self.layerNorm = nn.LayerNorm((d_model, len_max_seq))
        else:
    ...
    def forward(self, x):
        ...
        sz_b, d, seq_len, node = x.shape

        x = x.permute(0, 3, 2, 1).contiguous().view(sz_b*node, seq_len, d)  # (b, seq_len, d*node)
        sz_b, seq_len, d = x.shape
        x = self.inlayernorm(x)

        if self.inconv is not None:
            x = self.inconv(x.permute(0, 2, 1)) # torch.Size([4288, 256, 144])
            x = self.layerNorm(x).permute(0, 2, 1)  # (b, d_model, seq_len)
